...
function logger(func, logFunc) {
  return (...args) => {
    logFunc(`${func.name}(${args.map((item) => (Array.isArray ? JSON.stringify(item) : item))}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${args.map((item) => (Array.isArray ? JSON.stringify(item) : item))}) ends`);
